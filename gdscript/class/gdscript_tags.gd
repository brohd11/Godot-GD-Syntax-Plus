...
func _init() -> void:
	...
	editor_tags = GDHelper.read_from_json(JSON_PATH)
...
func _on_editor_script_changed(new_script:Script):
	...
	#if is_instance_valid(GDHelper) and is_instance_valid(GDHelper.base_gdscript_highlighter):
		#GDHelper.base_gdscript_highlighter.clear_highlighting_cache()
...
func _get_line_syntax_highlighting(line_idx: int) -> Dictionary:
	...
	if needs_sort:
		hl_info = GDHelper.sort_keys(hl_info)
