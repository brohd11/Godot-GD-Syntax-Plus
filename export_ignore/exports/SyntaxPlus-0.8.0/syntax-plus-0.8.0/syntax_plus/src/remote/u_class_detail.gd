...
static func _recur_get_class_members(script:Script, desired_members:=_MEMBER_ARGS):
	...
	var script_members = _get_script_members(base_script, members_array, desired_members)
	members_array.append_array(script_members)
...
static func script_get_all_members(script:Script, inh_class_members:Array=[]):
	if script == null:
		script = EditorInterface.get_script_editor().get_current_script()
	return _get_script_members(script, inh_class_members)
...
static func script_get_all_signals(script:Script, inh_class_members:Array=[]):
	...
	if script == null:
		return []
	var members = _get_script_members(script, inh_class_members, ["signal"])
	return members
...
static func script_get_all_properties(script:Script, inh_class_members:Array=[]):
	...
	if script == null:
		return []
	var members = _get_script_members(script, inh_class_members, ["property"])
	return members
...
static func script_get_all_methods(script:Script, inh_class_members:Array=[]):
	...
	if script == null:
		return []
	var members = _get_script_members(script, inh_class_members, ["method"])
	return members
...
static func script_get_all_constants(script:Script, inh_class_members:Array=[]):
	...
	if script == null:
		return []
	var members = _get_script_members(script, inh_class_members, ["const"])
	return members
...
static func _get_script_members(script:Script, inh_class_members, desired_members:=_MEMBER_ARGS):
	var members_array = []
	if "signal" in desired_members:
