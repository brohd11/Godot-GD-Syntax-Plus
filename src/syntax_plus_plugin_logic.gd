...
func _enter_tree() -> void:
	editor_plugin_manager = EditorPluginManager.new(plugin, EDITOR_PLUGINS_PATH, true)
	
	if not DirAccess.dir_exists_absolute(MODULAR_BROWSER_PATH): # If path present, use plugin layout
		plugin.add_tool_menu_item("SyntaxPlus", _on_tool_menu_pressed)
...
func _exit_tree() -> void:
	...
	plugin.remove_tool_menu_item("SyntaxPlus") # won't cause error if it doesn't exist
